...
export function renderTodos(project) {
  ...
  const todosHtml = project.todos
    .map(
      (todo) =>
        `<div class = "todo-item" data-todo-id = "${todo.id}" >
            <input type = "checkbox" ${todo.completed ? "checked" : ""}>
            <span>${todo.name} <br> Due: ${todo.dueDate || "N/A"} [${todo.priority}]</span>
...
export function bindAddTodo(handler) {
  ...
  const activeProjectName = document
    .querySelector("#project-details h2")
...
export function renderTodoDetailsForEdit(todo) {
  ...
  for (let i = 0; i < prioritySelect.options.length; i++) {
    if (
      prioritySelect.options[i].value.toLowerCase() ===
